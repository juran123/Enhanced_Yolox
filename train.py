...
if __name__ == "__main__":
    ...
    if model_path != '':
        ...
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if
                           model_dict.has_key(k) and np.shape(model_dict[k]) == np.shape(v)}
    ...
    if True:
        ...
        #   构建数据集加载器。
        # ---------------------------------------#
        train_dataset = YoloDataset(train_lines, input_shape, num_classes, epoch_length=UnFreeze_Epoch, train=False)
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            small_loss = fit_one_epoch(model_train, model, yolo_loss, loss_history, optimizer, epoch, epoch_step,
                                       epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, save_period, save_dir)
            gen.dataset.small_object_loss_now = small_loss
