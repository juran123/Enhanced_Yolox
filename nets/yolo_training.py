...
class YOLOLoss(nn.Module):
    def __init__(self, num_classes, fp16, strides=[8, 16, 32]):
        ...
        self.fp16 = fp16
        self.dynamic_loss_cls = []
        self.dynamic_loss_loc = []
        self.dynamic_loss_obj = []
        self.init_loss()
        self.w1 = 1.0
        self.w2 = 1.0
        self.w3 = 1.0
    ...
    def init_loss(self):
        for i in range(0, 3):
            self.dynamic_loss_cls.append(1.0)
            self.dynamic_loss_loc.append(1.0)
            self.dynamic_loss_obj.append(1.0)

    def process_weight(self, loss_ls):
        p = random.random()
        # return loss_ls[-1]/loss_ls[-2]
        if p > 0.5:
            return loss_ls[-1] / loss_ls[-2]
        else:
            return loss_ls[-2] / loss_ls[-3]

    def get_dynamic_weight(self):

        w1 = self.process_weight(self.dynamic_loss_loc)
        w2 = self.process_weight(self.dynamic_loss_obj)
        w3 = self.process_weight(self.dynamic_loss_cls)
        w1 = math.exp(w1)
        w2 = math.exp(w2)
        w3 = math.exp(w3)
        w_sum = w1 + w2 + w3
    ...
    def forward(self, inputs, labels=None, flag=True):
        outputs = []
        x_shifts = []
        y_shifts = []
        expanded_strides = []
        ...
        return self.get_losses(x_shifts, y_shifts, expanded_strides, labels, torch.cat(outputs, 1), flag)
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs, flag):
        # -----------------------------------------------#
        ...
        #             print("")
        #         print("损失")
        #         print(loss_iou.item(),loss_obj.item(),loss_cls.item())
        if flag:
            ww1, ww2, ww3 = self.get_dynamic_weight()
            #             print("权重先")
            #             print(ww1,ww2,ww3)
            loss = reg_weight * loss_iou * ww1 + loss_obj * ww2 + loss_cls * ww3
            self.dynamic_loss_obj.append(loss_obj.item() / num_fg)
            self.dynamic_loss_cls.append(loss_cls.item() / num_fg)
            self.dynamic_loss_loc.append(loss_iou.item() / num_fg)
        else:
            #             print("权重后")
            #             print(self.w1,self.w2,self.w3)
            loss = reg_weight * loss_iou * self.w1 + loss_obj * self.w2 + loss_cls * self.w3
        #         for _ in range(5):
        #             print("")
